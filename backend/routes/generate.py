...
from ..utils.auth import OptionalCurrentUserDep
...
async def generate_brd_endpoint(request: GenerateRequest, current_user=OptionalCurrentUserDep):
    """Generate a full BRD artifact set for the given idea. Supports authenticated and guest mode."""
    if not request.idea or not request.idea.strip():
        raise HTTPException(status_code=400, detail="Idea cannot be empty.")

    try:
        user_id = current_user.user_id if current_user else "guest"
        result = orchestrate_brd_generation(request.idea, user_id=user_id)
